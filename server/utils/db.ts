...
/**
 * Sync data from JSON sources to DB.
 * Can be called for initial seed or for live resync.
 */
export function syncDataFromJson(db: Database.Database) {
  // --- Agents ---
  ...
  if (projData?.projects) {
    ...
    for (const p of projData.projects) {
      ...
      // Clear old relationships before reinserting
      db.prepare('DELETE FROM project_agents WHERE project_id = ?').run(p.id)
      db.prepare('DELETE FROM project_phases WHERE project_id = ?').run(p.id)
      ...
      // Updates (only insert new ones, don't clear existing)
      if (p.updates) {
...
/**
 * Resync DB from JSON sources.
 * Can be called after file changes to update the DB.
 */
export function resyncFromJson() {
  const db = getDb()
  const tx = db.transaction(() => {
    syncDataFromJson(db)
  })
